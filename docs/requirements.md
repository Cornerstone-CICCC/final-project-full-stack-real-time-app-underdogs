...
The visitor's **name and ID** are remembered — never the conversation. Stored under
a single key `visitor`:

| Property | Example | Purpose |
|---|---|---|
| `visitorId` | `"a1b2c3d4-..."` | UUID generated on first visit; sent with every `POST /api/chats` request |
...
- `localStorage` has no built-in expiry, so we store `expiresAt` (now + 30 days)
  ...
  visitor as new.
- **`visitorId` is a UUID generated by the frontend on the visitor's first visit**
  and reused across subsequent visits. It is sent to the backend with each new
  ticket request so the server can record which visitor opened the ticket.
...
chats: [
  {
    ...
    visitorId,              // UUID from the frontend; identifies the visitor across tickets
    firstName,
    lastName,
    initials,               // e.g. "SM"
    status: 'open' | 'closed',
